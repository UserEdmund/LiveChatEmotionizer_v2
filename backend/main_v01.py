...
async def ws_chat(ws: WebSocket):
    ...
    try:
        while True:
            ...
            response = await run_in_threadpool(ollama.chat, user_text, percentages)
            
            ai_text = response.get("text", str(response)) if isinstance(response, dict) else str(response)
